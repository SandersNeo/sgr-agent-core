...
**RunCommandTool** настраивается через секцию `tools:`. Параметры: `workspace_path` (рабочая директория), `mode` (`"safe"` или `"unsafe"`, по умолчанию `"safe"`), `timeout_seconds` (по умолчанию 60), `include_paths` (опциональный список разрешённых команд/путей), `exclude_paths` (опциональный список запрещённых команд/путей). Приоритет у include_paths над exclude_paths. Режим safe использует Bubblewrap (bwrap) в сочетании с OverlayFS на Linux для точного исключения файлов через whiteout-файлы; bwrap должен быть установлен (например, `apt install bubblewrap`). Если bwrap не найден, тул возвращает ошибку со ссылкой на инструкцию по установке. При задании `include_paths`/`exclude_paths` OverlayFS создаёт отфильтрованные представления файловой системы, где исключённые бинарники скрыты через whiteout-файлы. Подробнее: [спецификация RunCommandTool и safe mode](../../specs/run-command-and-sandbox.md).
